...
def main():
    ...
    for env in envs:
        ...
        if delete_mode:
            ...
            deploy_charts(paths, env, dry_run=True)

            print("\nDeploying for: " + str(env))
            deploy_charts(paths, env)
...
def deploy_charts(paths, env, dry_run=False):
    for path in paths:
        deploy_chart(path, env, dry_run)


def deploy_chart(path, env, dry_run=False):
